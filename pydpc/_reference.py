...
class Cluster:
    ...
    def _get_halo(self):
        ...
        for i in range(self.npoints - 1):
            for j in range(i + 1, self.npoints):
                if (self.membership[i] != self.membership[j]) and (
                    self.distances[i, j] < self.kernel_size
                ):
        ...
        if self.border_only:
            for i in range(self.npoints):
                if (
                    self.density[i] < self.border_density[self.membership[i]]
                ) and self.border_member[i]:
                    self.halo[i] = -1
        else:
            for i in range(self.npoints):
